...
void gnu_point(int64_t a, int64_t b, int64_t col, Outputs &outs){
    std::string data_line = int64_to_string(a) + " " + int64_to_string(b) + "\n";
    if(col == 1) outs.point_pow_output += data_line;
    else if(col == 2) outs.point_mult_output += data_line;
    else if(col == 3) outs.point_div_output += data_line;
    outs.lines_output += outs.prev_point + "\n" +  int64_to_string(a) + " " + int64_to_string(b) + "\n\n";
    outs.prev_point = int64_to_string(a) + " " + int64_to_string(b);
}

// power
uint64_t pow_mod(uint64_t x, uint64_t y, uint64_t mod, Outputs &outs){
    if (x == 0 && y == 0) {
    ...
    for (y; y > 1; --y) {
    // multiplication
        gnu_point(r,x,2, outs); // 1 = green
        r = (r * x) % mod;
    }
    gnu_point(x,y,1, outs); // 1 = red
    return r;
}

//verif A: a^d =1 (mod n) ?
bool verif_a(uint64_t a, uint64_t d, uint64_t n, Outputs &outs){
    bool test_a = false;
    int r = pow_mod(a, d, n, outs);
    if ( r == 1) {
        test_a = true; 
    }
...
bool verif_b(uint64_t a, uint64_t d, uint64_t m, uint64_t n, Outputs &outs){
    bool test_b = false;
    for (uint64_t j=0; j <= m-1; j++){
        //std::cout << "result of test b: " << "a^((2^j)*d) = " << a << "^((2 ^ " << j << ")*" <<  d << ")= " << pow_mod(a, std::pow(2,j)*d, n) << "\n";
        // Power, multiplication
        uint64_t p2j = std::pow(2,j);
        gnu_point(2,j,1, outs); // 1 = red
        uint64_t var = p2j * d;
        gnu_point(p2j,d,2, outs); // 2 = green
        gnu_point(a,var,1, outs); // 1 = red
        if (pow_mod(a,var, n, outs) == n - 1) {
            test_b = true;
            break;
        }
    }  
...
int main(){
    ...
    std::cout << "please enter lower bound number to check for primality (must be odd and >= 5)\n";
    uint64_t low_n;
    std::cin >> low_n;
    std::cout << "please enter upper bound number to check for primality (must be odd)\n";
    uint64_t up_n;
    std::cin >> up_n;
    for(uint64_t n = low_n; n <= up_n ; n+= 2){
        uint64_t m;
        uint64_t d;
        Outputs outputs;
        ...
        for (uint64_t a = 0; a < star_witness.size(); a++){
            ...
            while( d % 2 == 0){
                ...
                // division
                d /= 2;
                gnu_point(d,2,3, outputs); // 3 = blue
            }
            //debug 
            //std::cout << n << " = " << " 2^ " << m << " * " << d << " + 1 \n";
            bool va = verif_a(star_witness[a],d,n, outputs);
            bool vb = verif_b(star_witness[a],d,m,n, outputs);
            if( va || vb ){
        ...
        myfile << outputs.point_pow_output;
        myfile.close();
        myfile.open ("multiplication_operations");
        myfile << outputs.point_mult_output;
        myfile.close();
        myfile.open ("division_operations");
        myfile << outputs.point_div_output;
        myfile.close();
        myfile.open ("order");
        myfile << outputs.lines_output;
        myfile.close();

        std::string cmd = "gnuplot -p -e \"graph_title = \'miller-rabin test visualisation for " + int64_to_string(n) + prime_str + "\'\" cmd_gnu.txt";
        system(cmd.c_str());
    }
    return 0; 
}